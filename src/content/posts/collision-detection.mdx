...
The main idea behind the Separating Axis Theorem is to find a line that can be drawn between two shapes without intersecting (a fancy word for "passing through") either of them. If you can find any line that can go between the two shapes – you've found yourself a separating axis. That means the two shapes don't intersect, which means that they don't collide. To efficiently find such a separating axis – one has to get a little bit creative and that's exactly what the SAT algorithm is all about and what these demos are trying to showcase step by step.

## How to Project a Line

The main idea necessary to pull all of this off is the idea of shadows! 

In linear algebra a projection is essentially what happens to a ball if you were to shine a light onto it from very far away. The ball would cast a shadow onto a wall at a specific location. 

The same idea can be applied to cast a shadow from a point (ball) onto a line (wall). The idea is pretty straightforward. 

Points are vectors, you can think of them as little arrows in space. A straight line can be defined by two points. 
...
The dot product of a vector is defined as adding up the multiples of it's components. Vectors can have multiple components, x,y,z etc. based on how many dimensions of space the vector is in, for these demos we keep everything super simple, and only worry about very simple 2D vectors with two components – x and y. 
