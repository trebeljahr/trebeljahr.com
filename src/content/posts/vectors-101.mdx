...
It's often useful to have the mag2 version because taking the square root is an expensive operation and for some applications, we don't need the "correct" length of the vector, because the squared length suffices. 
...
In 2D getting a vector rotated by exactly 90° to find its perpendicular is very easy! You can simply swap the x and y components, and then negate the swapped x!
$$
\begin{bmatrix}
x \\
y 
